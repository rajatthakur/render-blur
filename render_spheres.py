...
def render(viewpoint, temp_folder):
    """render rbg image 
    ...
    obj = bpy.data.objects['Cube_Cube.001']
    ...
    while tri <= g_max_trials:
        ...
        for ki in [0, ni-1]+list(range(1,ni-1)):
            ...
            if ki == 0 or ki == (ni-1):
                Mt = imageio.imread(os.path.join(bpy.context.scene.node_tree.nodes[1].base_path,'image-{:06d}.png'.format(ki+1)))[:,:,-1] > 0
...
def make_fmo(path, gt_path, video_path, h5_file):
    n_im = 5
    ...
    if jpg:
        ...
        B0 = imageio.imread(bgr_path)/255
    ...
    for ki in range(g_fmo_steps):
        FM[:,:,:,ki] = imageio.imread(os.path.join(gt_path,'image-{:06d}.png'.format(ki+1)))[:,:,[2,1,0,3]]/g_rgb_color_max
    ...
    for ki in range(1,n_im):
        ...
        if jpg:
            bgrkiimg = cv2.imread(bgrki_path)
        else:
            bgrkiimg = imageio.imread(bgrki_path)
        Ball[:,:,:,ki] = cv2.resize(bgrkiimg/255, dsize=(int(g_resolution_x*g_resolution_percentage/100), int(g_resolution_y*g_resolution_percentage/100)), interpolation=cv2.INTER_CUBIC)
    Ball[Ball > 1] = 1
    Ball[Ball < 0] = 0
    ...
    if h5_file is None:
        ...
        h5_file.create_dataset('im', data=Im[:,:,[2,1,0]], compression=compression_type)
        h5_file.create_dataset('bgr', data=B[:,:,[2,1,0]], compression=compression_type)
        h5_file.create_dataset('bgr_med', data=Bmed[:,:,[2,1,0]], compression=compression_type)
        h5_file.create_group("GT")
        for ki in range(g_fmo_steps):
            gtsavename = 'image-{:06d}'.format(ki+1)
            h5_file["GT"].create_dataset(gtsavename, data=FM[:,:,[2,1,0,3],ki], compression=compression_type)
...
def render_obj(path, name, temp_folder, h5_file, apply_texture=g_apply_texture):
    ...
    random.shuffle(vps)
    if path is not None:
        save_path = os.path.join(path,"im.png")
        gt_path = os.path.join(path)
        video_path = os.path.join(path,"video.avi")
    else:
        save_path = os.path.join(temp_folder,"im.png")
        gt_path = os.path.join(temp_folder)
        video_path = os.path.join(temp_folder,"video.avi")
    ...
    while tri <= g_max_trials:
        ...
        sample_different_object, sample_different_vp = render(vp, temp_folder)
        ...
        if sample_different_vp:
            ...
            continue
        success = make_fmo(save_path, gt_path, video_path, h5_file)
        if success:
            return True
...
h5_file = None
if 'hdf5' in g_syn_rgb_folder:
    if os.path.exists(g_syn_rgb_folder):
        os.remove(g_syn_rgb_folder)
    h5_file = h5py.File(g_syn_rgb_folder)
    h5_file.create_group("sphere")
    temp_folder = dataset_folder+'/tmp/'
    if not os.path.exists(temp_folder):
        os.mkdir(temp_folder)
else:
    temp_folder = g_syn_rgb_folder+'/'

for objid in range(g_number):
    name = "sphere_{:04d}".format(objid+1)
    if h5_file is not None:
        h5_file["sphere"].create_group(name)
        obj_folder = None
    else:
        obj_folder = os.path.join(g_syn_rgb_folder, name)
        if not os.path.exists(obj_folder):
            os.makedirs(obj_folder)
    print("Processing {}".format(name))
    clear_mesh()
    old = open_log(temp_folder)
    bpy.ops.import_scene.obj(filepath=g_sphere_path, axis_forward='-Z', axis_up='Y', filter_glob="*.obj;*.mtl", use_split_groups=False, use_split_objects=True)
    # bpy.ops.import_scene.obj(filepath=path)
    close_log(old)
    #combine_objects()
    #scale_objects(0.5)
    if h5_file is not None:
        if g_number_frames > 1:
            apply_texture = True
            for frmi in range(g_number_frames):
                frm_name = "{:01d}".format(frmi+1)
                print("   frame {}".format(frm_name))
                h5_file["sphere"][name].create_group(frm_name)
                result = render_obj(obj_folder, name, temp_folder, h5_file["sphere"][name][frm_name], apply_texture)
                apply_texture = False
        else:
            result = render_obj(obj_folder, name, temp_folder, h5_file["sphere"][name])
    else:
        result = render_obj(obj_folder, name, temp_folder, h5_file)
