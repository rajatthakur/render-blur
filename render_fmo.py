...
def make_fmo(path, gt_path, video_path, h5_file):
    ...
    if h5_file is None:
        ...
        h5_file.create_dataset('im', data=Im, compression=compression_type)
        h5_file.create_dataset('bgr', data=B, compression=compression_type)
        h5_file.create_dataset('bgr_med', data=Bmed, compression=compression_type)
        h5_file.create_group("GT")
        for ki in range(g_fmo_steps):
            gtsavename = 'image-{:06d}'.format(ki+1)
            h5_file["GT"].create_dataset(gtsavename, data=FM[:,:,:,ki], compression=compression_type)
...
def render_obj(obj_path, path, objid, obj_name, temp_folder, h5_file):
    ...
    if path is not None:
        ...
        save_path = os.path.join(temp_folder,"{}_{:04d}.png".format(obj_name,objid))
        gt_path = os.path.join(temp_folder,"GT")
        video_path = os.path.join(temp_folder,"{}_{:04d}.avi".format(obj_name,objid))
...
for obj_name in g_render_objs[start_index:(start_index+step_index)]:
    ...
    while objid <= g_number_per_category:
        if h5_file is not None:
            name = "{}_{:04d}".format(obj_name,objid)
            h5_file[obj_name].create_group(name)
        ...
        #scale_objects(0.5)
        if h5_file is not None:
            result = render_obj(path, obj_folder, objid, obj_name, temp_folder, h5_file[obj_name][name])
        else:
            result = render_obj(path, obj_folder, objid, obj_name, temp_folder, h5_file)
        if result:
            objid += 1
            tri = 0
        else:
